...
- **@todos/firebase Owns Firebase Infrastructure**: `@todos/firebase` owns Firebase app setup, auth/firestore services (including user creation via `FirebaseAuthService.createUser()`, token verification, and user lookups), and Firebase repository adapters that implement `@todos/core` contracts. All Firebase Admin SDK operations must go through this package; `apps/**` must not initialise or import the Firebase Admin SDK directly.
